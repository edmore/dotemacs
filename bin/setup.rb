#! /usr/bin/ruby

##########################################################
# Description - Setup emacs configuration
# Author - www.edmoremoyo.com
##########################################################

home = File.expand_path('~')
backup_directory = "#{home}/.emacs.d_backup/"
cmds = []

cmds << "sudo mkdir #{backup_directory}"
# Backup the current emacs dotfiles if they exist
cmds << "sudo mv #{home}/.emacs #{backup_directory}" if File.exist?("#{home}/.emacs")
cmds << "sudo mv #{home}/.emacs.d/ #{backup_directory}.emacs.d/" if File.directory?("#{home}/.emacs.d/")
# Rename dotfiles folder to .emacs.d
cmds << "sudo mv #{home}/dotemacs/ #{home}/.emacs.d/"
# update submodules
cmds << "git submodule update --init"

system cmds.join("&&")
puts "Setup complete :-) , you may navigate to [ ~/.emacs.d/ ] to view your GNU Emacs config files."
